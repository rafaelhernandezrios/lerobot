#!/usr/bin/env python3
"""
Guided setup for bimanual SO-101: map 4 serial arms + 3 OpenCV USB cameras, optional calibration,
and print a `lerobot-teleoperate` command (`bi_so101_*`).

Uses the same unplug/replug idea as `map_so101_ports.py` for serial devices and for cameras
(OpenCV index or `/dev/video*` on Linux), matching devices by appearance/disappearance.
"""
...
def calibrate_bimanual(
    ...
    if run_cmd(leaders_cmd()) != 0:
        print("Leader (master) bimanual calibration failed.")
        return False
    if run_cmd(followers_cmd()) != 0:
...
def preview_cameras(name_to_id: dict[str, int | str], seconds: float) -> None:
    ...
    while time.time() < t_end:
        ...
        if not frames:
            ...
            return
        # Horizontal stack (resize to same height).
        target_h = 360
        resized = []
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Map 4 SO-101 serial ports + 3 USB cameras, optional calibration, print teleop command."
    )
    ...
    print("SO101 bimanual + cameras mapper")
    print(f"Platform: {platform.system()} — initial serial ports: {list_arm_serial_ports() or 'none'}")
    try:
        print(f"Initial OpenCV cameras: {sorted(get_opencv_id_set(), key=str)}")
    except Exception as err:
        print(f"Could not list OpenCV cameras (is OpenCV installed?): {err}")
    ...
    if ports:
        for k, v in ports.items():
            print(f"{k}_PORT={v}")
    if cam_map:
        print("Cameras (OpenCV index_or_path):", cam_map)
    ...
    if ports and cam_map:
        ...
        ex_cam = build_cameras_json(
            {"cam_a": 0, "cam_b": 1, "cam_c": 2},
            args.cam_width,
            args.cam_height,
            args.cam_fps,
        )
